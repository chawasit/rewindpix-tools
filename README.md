...
| [Presets](src/presets.html) | Edit the camera's 3 **film** slots (names + 7 params) and 3 **in-camera** slots (override / keep-baked), **set the roll size** (frame budget), apply to the camera, and save presets. **Name a film slot after a LUT** (`LUT ▾` picker) so shots taken on that slot auto-preview with the matching LUT in the gallery. Every slot shows a live **Example** rendered on a real photo — pick the **Example photo** from bundled samples or **choose one from the gallery** (camera shot); LUT-named slots get a **↺ defaults** button that resets the 7 params (official LUTs are tuned for defaults). |
